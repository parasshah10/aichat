...
// const openIdClient = require('openid-client'); // Disabled due to ES module compatibility
...
const refreshController = async (req, res) => {
  ...
  if (token_provider === 'openid' && isEnabled(process.env.OPENID_REUSE_TOKENS) === true) {
    // OpenID client disabled due to ES module compatibility issue
    logger.warn('[refreshController] OpenID refresh token functionality disabled');
    return res.status(403).send('OpenID refresh token functionality temporarily disabled');
  }
  try {
