...
// NEW: Helper function to check for text-based file types to bypass OCR
/**
 * A set of common text-based file extensions that do not require OCR.
 */
...
/**
 * Checks if a file is a text-based format that can bypass OCR.
 * @param file The file object from Multer.
 * @returns True if the file is a text-based format, false otherwise.
 */
function isTextBasedFile(file: Express.Multer.File): boolean {
  if (!file.originalname) {
    return false;
  }
  const extension = path.extname(file.originalname).slice(1).toLowerCase();
  return TEXT_BASED_EXTENSIONS.has(extension);
}
...
export const uploadMistralOCR = async (context: OCRContext): Promise<MistralOCRUploadResult> => {
  try {
    // NEW: Check if the file is a text-based format to bypass OCR.
    if (isTextBasedFile(context.file)) {
      logger.info(
        `[uploadMistralOCR] Bypassing OCR for text-based file: ${context.file.originalname}`,
      );
      const text = fs.readFileSync(context.file.path, 'utf-8');
      return {
        filename: context.file.originalname,
        // Use the actual file size for bytes.
        bytes: context.file.size,
        // Using a custom filepath to indicate direct text extraction.
        // This could be added to the FileSources enum for consistency.
        filepath: 'text_extraction',
        text,
        images: [],
      };
    }
...
export const uploadAzureMistralOCR = async (
  ...
  try {
    ...
    if (isTextBasedFile(context.file)) {
      logger.info(
        `[uploadAzureMistralOCR] Bypassing OCR for text-based file: ${context.file.originalname}`,
      );
      const text = fs.readFileSync(context.file.path, 'utf-8');
      return {
        filename: context.file.originalname,
